...
public class getSliderValueMQ9 : MonoBehaviour
{
    ...
    public void Duplicate()
    {
        if (diff > 0)
        {
            objectToDuplicate = GameObject.Find("Aircraft");
            Instantiate(objectToDuplicate, new Vector3(i * 30.0F, 0 * 30.0F + 400.0f, 0 * 30.0F), Quaternion.identity);
            i++;
        }
    ...
    public void Delete()
    {
        if (diff < 0)
        {
            objectToDuplicate = GameObject.Find("Aircraft(Clone)");
